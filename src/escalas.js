export const escalaMixagem = {
	title: "Escala da Mixagem",
	positions: ['mesa de som'],
	people: [
		{
			name: 'gabriel',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'domingo'],
			pos: ['mesa de som']
		},
		{
			name: 'silas',
			days: ['quarta-feira', 'sábado'],
			pos: ['mesa de som']
		},
		{
			name: 'luis',
			days: ['quinta-feira', 'sexta-feira'],
			pos: ['mesa de som']
		},
		{
			name: 'alessandra',
			days: ['terça-feira'],
			pos: ['mesa de som']
		}
	]
}

export const escalaLouvor = {
	title: "Escala do Louvor",
	positions: ['voz principal', 'back vocal', 'bateria', 'baixo', 'guitarra', 'violão', 'teclado', 'sopro'],
	people: [
		{
			name: 'felipe',
			days: ['terça-feira', 'quarta-feira', 'domingo'],
			pos: ['voz principal', 'teclado', 'back vocal']
		},
		// preenchimento invisível de terça-feira
		{
			name: ' ',
			days: ['sexta-feira'],
			pos: ['teclado']
		},
		{
			name: '',
			days: ['terça-feira'],
			pos: ['voz principal', 'violão', 'guitarra', 'baixo', 'bateria']
		},
		{
			name: ' ',
			days: ['terça-feira'],
			pos: ['back vocal', 'violão', 'guitarra', 'baixo', 'bateria']
		},
		{
			name: '  ',
			days: ['terça-feira'],
			pos: ['back vocal', 'violão', 'guitarra', 'baixo', 'bateria']
		},
		{
			name: '    ',
			days: ['terça-feira'],
			pos: ['back vocal', 'violão', 'guitarra', 'baixo', 'bateria']
		},
		{
			name: '     ',
			days: ['terça-feira'],
			pos: ['violão', 'guitarra', 'baixo', 'bateria' , 'sopro']
		}, 
		// Instrumentistas
		{
			name: 'Peck',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['violão', 'baixo']
		},
		{
			name: 'Maicon',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['violão', 'baixo']
		},
		{
			name: 'Helder',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['violão', 'baixo']
		},
		{
			name: 'Monique',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['violão', 'back vocal']
		},
		{
			name: 'Talisson',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['guitarra']
		},
		{
			name: 'Fernando',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['guitarra']
		},
		{
			name: 'Bruno',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['baixo']
		},
		{
			name: 'Joao Paulo',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['baixo']
		},
		{
			name: 'Pablo',
			days: ['quinta-feira', 'sábado'],
			pos: ['teclado']
		},
		{
			name: 'Pedrinho',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['bateria']
		},
		{
			name: 'Luan',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['bateria']
		},
		{
			name: 'George',
			days: ['sexta-feira', 'domingo'],
			pos: ['bateria']
		},
		{
			name: 'Jailton',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['sopro']
		},
		{
			name: 'Abner',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['sopro']
		},
		{
			name: ' ',
			days: ['quinta-feira'],
			pos: ['sopro']
		},
		// vozes principais
		{
			name: 'Laís',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['voz principal', 'back vocal']
		},
		{
			name: 'Isabele',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['voz principal', 'back vocal']
		},
		{
			name: 'Alberto',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['voz principal', 'back vocal']
		},
		{
			name: 'Luana',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['voz principal', 'back vocal']
		},
		{
			name: 'Guew',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['voz principal']
		},
		{
			name: 'Gislaine',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['voz principal']
		},
		{
			name: 'Andreia',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['voz principal', 'back vocal']
		},
		{
			name: 'Neiva',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['voz principal', 'back vocal']
		},
		// Back Vocal
		{
			name: 'Isac',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['back vocal']
		},
		{
			name: 'Rebeca',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['back vocal']
		},
		{
			name: 'Cecília',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['back vocal']
		},
		{
			name: 'Lara',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['back vocal']
		},
		{
			name: 'Ryan',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['back vocal']
		},
		{
			name: 'Andriele',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['back vocal']
		},
		{
			name: 'Eddye',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['back vocal']
		},
		{
			name: 'Juliana',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['back vocal']
		},
		{
			name: 'Geise',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['back vocal']
		},
	]
}

export const escalaFilmagem = {
	title: "Escala da Filmagem",
	positions: ['mesa de corte', 'joystick', 'notebook', 'camera 4'],
	people: [
		{
			name: 'Marcelo',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['mesa de corte', 'joystick', 'notebook']
		},
		{
			name: 'Gleidson',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['mesa de corte', 'joystick', 'notebook']
		},
		{
			name: 'Bruno',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['mesa de corte', 'joystick', 'notebook']
		},
		{
			name: 'Edmilson',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['mesa de corte', 'joystick', 'notebook']
		},
		{
			name: 'Luis',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['mesa de corte', 'joystick', 'notebook']
		},
		{
			name: 'Naldo',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['camera 4']
		},
		{
			name: 'Vanailton',
			days: ['quarta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['mesa de corte', 'joystick', 'notebook']
		},
		{
			name: 'Leo',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['mesa de corte', 'joystick', 'notebook']
		},
		{
			name: 'Telles',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['mesa de corte', 'joystick', 'notebook']
		},
		{
			name: 'Gabriel',
			days: ['terça-feira', 'quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['mesa de corte', 'joystick', 'notebook']
		},
		{
			name: 'Ademário',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['camera 4']
		},
		{
			name: 'Matheus',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['camera 4']
		},
		{
			name: 'Kailan',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['camera 4']
		},
		{
			name: 'Michel',
			days: ['quarta-feira', 'quinta-feira', 'sexta-feira', 'sábado', 'domingo'],
			pos: ['mesa de corte', 'joystick', 'notebook']
		},
		{
			name: 'Ronald',
			days: ['sábado', 'domingo'],
			pos: ['mesa de corte', 'joystick', 'notebook']
		},
		
	]
}